...
import { FaEye, FaTruck, FaCheck } from "react-icons/fa";
import { Badge, Card, Table, Form, InputGroup } from "react-bootstrap";
import "../scss/OrderList.scss";

const VendedorOrderList = ({ statusFilter = null }) => {
    ...
    const fetchOrders = async () => {
        try {
            setLoading(true);
            let endpoint = "/vendedor/listar-pedidos/";
            if (statusFilter) {
                endpoint = `/vendedor/listar-pedidos/?status=${statusFilter}`;
            }
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        try {
            await api.patch(`/vendedor/actualizar-estado-pedido/${orderId}/`, {
                status: newStatus,
            });
                                        ...
                                        <tr key={order.id}>
                                            ...
                                            <td>
                                                <div className="action-buttons">
                                                    <Link to={`/vendedor/pedidos/${order.id}`} className="btn btn-sm btn-info me-1">
                                                        <FaEye />
                                                    </Link>
...
export default VendedorOrderList;